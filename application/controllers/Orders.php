...
class Orders extends Admin_Controller 
{
    ...
    */
    public function exportCsv()
    {
        ...
        $header = array("ID", "Bill No", "Customer Name", "Phone", "Date Time", "Total Products", "Gross Amount", "Net Amount", "Paid Status");
        fputcsv($file, $header);

        foreach ($orders as $key => $line) {
            $count_total_item = $this->model_orders->countOrderItem($line['id']);
            $date_time = date('Y-m-d H:i:s', (int)$line['date_time']);
            $status = ($line['paid_status'] == 1) ? 'Paid' : 'Unpaid';
