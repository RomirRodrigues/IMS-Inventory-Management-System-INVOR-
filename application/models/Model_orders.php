...
class Model_orders extends CI_Model
{
	...
	{
		$sql = "SELECT SUM(CAST(net_amount AS DECIMAL(10,2))) as total_revenue FROM `orders` WHERE paid_status = ?";
		$query = $this->db->query($sql, array(1));
	...
	public function getMonthlySalesData($year = null)
	{
		...
		$sql = "SELECT date_time, net_amount FROM `orders` WHERE paid_status = 1";
		$query = $this->db->query($sql);
		$result = $query->result_array();

		foreach ($result as $row) {
			if (!empty($row['date_time'])) {
				$order_year = date('Y', (int)$row['date_time']);
				if ($order_year == $year) {
					$month = (int)date('n', (int)$row['date_time']);
					$monthly_data[$month] += (float)$row['net_amount'];
				}
			}
		}
