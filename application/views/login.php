...
<body class="bg-black text-slate-300 antialiased min-h-screen flex flex-col selection:bg-slate-700 selection:text-white relative" style="font-family: 'Inter', sans-serif;">
    ...
    <main class="flex-grow flex flex-col items-center justify-start relative z-30 px-4 pt-4 pb-8 min-h-screen w-full">
        
        <!-- Gateway Card with Hover Border Gradient -->
        <div class="max-w-md w-full bg-black/95 backdrop-blur-xl rounded-2xl p-6 md:p-7 shadow-2xl flex flex-col relative group border border-white/[0.08]">
            ...
            <div class="text-center mb-5 w-full relative z-20">
                <div class="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-black/50 border border-slate-700/50 mb-3 shadow-inner">
                    <iconify-icon icon="solar:cpu-bolt-linear" width="22" height="22" stroke-width="1.5" class="text-slate-200"></iconify-icon>
                </div>
                <h1 id="reveal-title" class="text-2xl md:text-3xl font-thin tracking-tight text-white leading-tight mb-1 uppercase flex flex-wrap justify-center gap-x-2">
            ...
            <div class="mb-5 p-3 rounded-xl bg-sky-950/40 border border-sky-500/30 text-xs relative z-20 flex flex-col gap-1.5">
            ...
            <div class="grid grid-cols-2 gap-1 p-1 bg-slate-950 border border-slate-800 rounded-xl mb-5 relative z-20">
            ...
            <form id="login-form-box" action="<?php echo base_url('auth/login'); ?>" method="post" class="space-y-4 relative z-20">
                <div>
                    <label for="email" class="text-xs font-light text-slate-400 mb-1 block uppercase tracking-widest">Email or Username</label>
                    <div class="relative rounded-lg bg-black/80">
                        <input type="text" id="email" name="email" required class="w-full bg-transparent px-3 py-2 text-sm text-slate-200 focus:outline-none border border-slate-800 rounded-lg focus:border-slate-500 font-extralight select-text" placeholder="admin@admin.com">
                    </div>
                </div>
                <div>
                    <div class="flex justify-between items-center mb-1">
                        <label for="password" class="text-xs font-light text-slate-400 block uppercase tracking-widest">Security Password</label>
                    </div>
                    <div class="relative rounded-lg bg-black/80">
                        <input type="password" id="password" name="password" required class="w-full bg-transparent px-3 py-2 text-sm text-slate-200 focus:outline-none border border-slate-800 rounded-lg focus:border-slate-500 font-extralight select-text" placeholder="••••••••">
                    </div>
                </div>
            ...
            <form id="register-form-box" action="<?php echo base_url('auth/register'); ?>" method="post" class="space-y-3 relative z-20 hidden">
                <div class="grid grid-cols-2 gap-2">
                    <div>
                        ...
                        <input type="text" name="firstname" required class="w-full bg-transparent px-3 py-1.5 text-xs text-slate-200 border border-slate-800 rounded-lg focus:border-slate-500 font-extralight select-text" placeholder="John">
                    </div>
                    <div>
                        <label class="text-[11px] font-light text-slate-400 mb-1 block uppercase tracking-wider">Last Name</label>
                        <input type="text" name="lastname" required class="w-full bg-transparent px-3 py-1.5 text-xs text-slate-200 border border-slate-800 rounded-lg focus:border-slate-500 font-extralight select-text" placeholder="Doe">
                    </div>
                </div>
                <div>
                    <label class="text-[11px] font-light text-slate-400 mb-1 block uppercase tracking-wider">Email Address</label>
                    <input type="email" name="email" required class="w-full bg-transparent px-3 py-1.5 text-xs text-slate-200 border border-slate-800 rounded-lg focus:border-slate-500 font-extralight select-text" placeholder="user@company.com">
                </div>
                <div>
                    <label class="text-[11px] font-light text-slate-400 mb-1 block uppercase tracking-wider">Choose Username</label>
                    <input type="text" name="username" required class="w-full bg-transparent px-3 py-1.5 text-xs text-slate-200 border border-slate-800 rounded-lg focus:border-slate-500 font-extralight select-text" placeholder="johndoe">
                </div>
                <div>
                    <label class="text-[11px] font-light text-slate-400 mb-1 block uppercase tracking-wider">Create Security Password</label>
                    <input type="password" name="password" required class="w-full bg-transparent px-3 py-1.5 text-xs text-slate-200 border border-slate-800 rounded-lg focus:border-slate-500 font-extralight select-text" placeholder="••••••••">
                </div>
            ...
            <div class="z-20 flex flex-col items-center">
                <button type="button" onclick="triggerGoogleSSO()" class="relative flex items-center justify-center gap-3 w-full bg-black/60 hover:bg-slate-900 border border-slate-800 hover:border-slate-600 rounded-lg py-2 text-xs text-slate-200 transition-all font-light group/alt shadow-md">
